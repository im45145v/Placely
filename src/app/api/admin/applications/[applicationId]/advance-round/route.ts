...
import { advanceApplicationFromRoundForAdmin } from "@/lib/applications/service";
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(await advanceApplicationFromRoundForAdmin(actor, applicationId, body.roundId, body.notes));
  } catch (error) {
    if (error instanceof AppError) {
      return NextResponse.json({ error: error.code, message: toUserMessage(error), details: error.details }, { status: error.statusCode });
    }
    console.error("[api/admin/applications/[applicationId]/advance-round]", error);
