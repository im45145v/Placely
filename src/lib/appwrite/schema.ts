...
export const APPWRITE_DATABASE_SCHEMA: {
  ...
  collections: [
    ...
    collection(Collections.ROUND_PARTICIPANTS, "RoundParticipant", true, [
      ...
      field("slotLabel", "string", false, "Optional human-readable slot label."),
      field("room", "string", false, "Optional room or cabin identifier."),
      ...
      field("scheduleTimezone", "string", false, "IANA timezone used for scheduling."),
      field("scheduleStatus", "enum", true, "pending | scheduled | rescheduled | cancelled", ["pending", "scheduled", "rescheduled", "cancelled"]),
      field("cancellationReason", "string", false, "Optional schedule cancellation reason."),
      ...
      field("lastScheduledAt", "datetime", false, "Latest schedule or reschedule timestamp."),
      timestamp("createdAt"),
      timestamp("updatedAt"),
    ], [
      ...
      index("participant_round_start", "key", ["roundId", "scheduledStart"], "Round schedule planning."),
    ...
    collection(Collections.EMAIL_DELIVERIES, "EmailDelivery", true, [
      relation("userId", Collections.USERS, "many-to-one", true, "Recipient user."),
      relation("universityId", Collections.UNIVERSITIES, "many-to-one", true, "Owning university."),
      field("notificationType", "string", true, "Notification type key."),
      field("templateKey", "string", true, "Template identifier used to render this email."),
      field("dedupeKey", "string", true, "Stable key to prevent duplicate deliveries."),
      field("provider", "enum", true, "Email delivery provider.", ["google_apps_script", "log"]),
      field("status", "enum", true, "pending | sending | retrying | sent | failed", ["pending", "sending", "retrying", "sent", "failed"]),
      field("toEmail", "string", true, "Recipient email address."),
      field("subject", "string", true, "Rendered email subject."),
      field("title", "string", true, "Rendered email title."),
      field("body", "string", true, "Rendered email body."),
      field("payload", "json", false, "Structured provider payload."),
      field("attempts", "integer", true, "Total delivery attempts made."),
      field("maxAttempts", "integer", true, "Maximum allowed delivery attempts."),
      field("lastAttemptAt", "datetime", false, "Latest delivery attempt timestamp."),
      field("nextAttemptAt", "datetime", false, "When the next retry becomes eligible."),
      field("sentAt", "datetime", false, "Successful delivery timestamp."),
      field("providerMessageId", "string", false, "Provider-side message identifier."),
      field("providerResponse", "json", false, "Last successful provider response."),
      field("lastError", "string", false, "Last delivery failure message."),
      timestamp("createdAt"),
      timestamp("updatedAt"),
    ], [
      index("email_delivery_dedupe_unique", "unique", ["dedupeKey"], "Prevents duplicate email dispatch for the same event and recipient."),
      index("email_delivery_status_next_attempt", "key", ["status", "nextAttemptAt"], "Scans due retries efficiently."),
      index("email_delivery_user_created", "key", ["userId", "createdAt"], "Supports recipient-level delivery history."),
    ], [
      permit("placement_admin", "read", "Placement admins may inspect operational email deliveries in their university."),
      permit("super_admin", "read", "Super admins may inspect all email deliveries."),
      permit("system", "create", "Email delivery records are created by trusted server workflows."),
      permit("system", "update", "Email delivery status transitions are managed by trusted server workflows."),
    ]),
    ...
    collection(Collections.AUDIT_LOGS, "AuditLog", true, [
      ...
      field("actorName", "string", false, "Actor display name at event time."),
      field("actorEmail", "string", false, "Actor email at event time."),
      ...
      index("audit_action_timestamp", "key", ["action", "timestamp"], "Action-level review."),
    ], [
